...
#: Шаги, которым стоит выполниться ЗАНОВО, если оценка появилась уже после
#: их завершения (модель чинили, задачу вернули через requeue).
#:
#: Sheets здесь есть: дозапись пустых ячеек в существующую строку невидима
#: и полезна. Telegram — НЕТ, и это осознанно: повторное уведомление — это
#: второе сообщение владельцу, то есть спам. Цена та же, что была при выборе
#: «дубль дешевле пропуска», только с обратным знаком: пропуск обогащения
#: дешевле дубля сообщения.
AI_REFRESH_STEPS: list[str] = ["sheets"]
